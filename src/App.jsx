...
export default function App() {
  ...
  const logLocalPvpMatch = (result, opponentName, myScore, opponentScore) => {
    ...
    const localHistory = localStorage.getItem(`thebongda_pvp_history_${currentUser}`) 
      ? JSON.parse(localStorage.getItem(`thebongda_pvp_history_${currentUser}`)) 
      : [];
  ...
  return (
    <>
        ...
        <div className="pvp-lobby relative z-10 p-4 sm:p-8 pt-20 min-h-screen flex flex-col max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-black/60 p-5 rounded-3xl border border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
            <div className="flex flex-wrap gap-3 items-center">
              <button 
                className="btn !bg-blue-600 hover:!bg-blue-500 !py-2 !px-4 text-xs font-black uppercase tracking-wider flex items-center gap-1.5 rounded-full border border-white/10 cursor-pointer shadow-md" 
            ...
            <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-2xl border border-white/10 shadow-inner">
              ...
              <div className="flex items-center gap-1 text-[10px] bg-white/5 border border-white/10 px-2 py-0.5 rounded-md font-bold text-gray-300">
                ...
                {(() => {
                  const tier = getPlayerTier(level);
                  return (
                    <span className={`text-[8px] font-black uppercase ml-1 px-1 rounded-sm border ${tier.color} ${tier.glow}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full flex-1">
                ...
                {onlineUsers.length === 0 ? (
                  ...
                  onlineUsers.map((user) => {
                    ...
                    return (
                      <div 
                        key={user.username}
                        className="p-4 bg-black/40 rounded-2xl border border-white/5 flex flex-col gap-3 hover:border-cyan-500/30 hover:bg-black/60 transition-all duration-300 relative overflow-hidden group shadow-md"
                      >
                        {/* Profile Info Row */}
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <button 
                            className="flex-1 btn !py-2 !px-3 text-[10px] flex items-center justify-center gap-1 bg-slate-800 hover:bg-slate-700 text-white shadow-md transition-all uppercase font-black tracking-widest cursor-pointer border border-white/10 rounded-xl"
                            onClick={() => {
                              playFx('click');
                              setActivePrivatePartner(user.username);
                              setChatTab('private');
                          ...
                          <button 
                            className={`flex-1 btn !py-2 !px-3 text-[10px] flex items-center justify-center gap-1 shadow-md transition-all uppercase font-black tracking-widest cursor-pointer rounded-xl ${
                              squad.length < 11
                                ? 'opacity-40 !bg-gray-700 cursor-not-allowed text-gray-400 border border-transparent' 
                                : 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white hover:scale-[1.03] border border-emerald-400/20 shadow-[0_0_15px_rgba(16,185,129,0.25)]'
                            }`}
                            disabled={squad.length < 11}
                            onClick={() => {
                              if (squad.length === 11) {
                      ...
                      {chatMessages.length === 0 ? (
                        ...
                        chatMessages.map((msg) => {
                          ...
                          return (
                            ...
                            >
                              {isSystem ? (
                                <div className="w-7 h-7 rounded-full shrink-0 flex items-center justify-center text-[10px] bg-gradient-to-br from-amber-500 to-yellow-600 border border-amber-500/30 shadow-md select-none animate-scale-in">📣</div>
                              ) : (
                                <div 
                                  className="w-7 h-7 rounded-full shrink-0 flex items-center justify-center text-[10px] font-black text-white border border-white/10 shadow-md cursor-pointer transition-all hover:scale-105 active:scale-95 select-none animate-scale-in"
                                  style={{ background: getAvatarGradient(msg.sender) }}
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden p-3 sm:p-4 animate-fade-in">
                    {activePrivatePartner ? (
                      <div className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-2">
                          <button 
                            className="flex items-center gap-0.5 text-[10px] font-bold text-violet-400 hover:text-violet-300 cursor-pointer animate-pulse"
                            onClick={() => setActivePrivatePartner(null)}
                          >
                            <ChevronLeft size={14} /> Trở lại
                          </button>
                        ...
                        >
                          <input 
                            id="pvp-lobby-private-chat-input"
                            type="text" 
                            className="flex-1 bg-transparent border-0 focus:outline-none focus:ring-0 text-xs px-3 text-white"
                            placeholder={`Nhắn cho ${activePrivatePartner}...`}
                            value={privateChatInput}
            ...
            <div className="glass-panel p-5 rounded-3xl flex flex-col h-[580px] border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.4)] bg-slate-950/40 overflow-hidden relative">
              ...
              <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-3.5 hide-scrollbar">
                {pvpHistory.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-gray-500 text-xs py-12 italic text-center leading-relaxed">
                  ...
                  pvpHistory.map((record) => {
                    ...
                    const badgeStyle = isWin 
                      ? 'bg-emerald-950/40 border-emerald-500/30 text-emerald-400 drop-shadow-[0_0_8px_rgba(16,185,129,0.3)] font-black' 
                      : isLose 
                      ? 'bg-rose-950/40 border-rose-500/30 text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.3)] font-black' 
                      : 'bg-blue-950/40 border-blue-500/30 text-blue-400 drop-shadow-[0_0_8px_rgba(59,130,246,0.3)] font-black';
                    ...
                    return (
                      <div 
                        key={record.id} 
                        className={`p-4 rounded-2xl border flex flex-col gap-3.5 transition-all duration-300 hover:scale-[1.02] bg-black/40 border-white/5 shadow-sm hover:border-white/10`}
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-gray-500 font-semibold font-mono">
                            {record.timestamp ? new Date(record.timestamp).toLocaleString('vi-VN', {hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit'}) : 'Vừa xong'}
                          </span>
                        </div>
              ...
              <div className="glass-panel p-6 sm:p-10 rounded-3xl text-center w-full max-w-md border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex flex-col gap-3 mt-4 w-full">
                    <button 
                      className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-500 hover:to-green-500 text-white font-black uppercase tracking-wider text-xs sm:text-sm py-3.5 px-4 rounded-xl border border-emerald-400/20 shadow-[0_0_15px_rgba(16,185,129,0.25)] transition-all cursor-pointer active:scale-95 animate-pulse"
                      onClick={() => {
                        playFx('click');
                        setActiveShareData({
                          type: 'ai',
      ...
      {activeShareData && (() => {
        const { type, result, myScore, opponentScore, opponentName } = activeShareData;
        ...
        return (
          <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 p-4 animate-fade-in" onClick={() => setActiveShareData(null)}>
            <div 
            ...
            >
              <button 
                ...
                onClick={() => { playFx('click'); setActiveShareData(null); }}
              >
                ✕
              </button>
